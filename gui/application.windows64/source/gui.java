...
public void createMainMenu() {
  ...
  for (int i = 0; i < 8; i++) {
    cp5.addButton("Lane" + PApplet.parseChar(49+i))
      .setFont(bFont)
  ...
  cp5.addButton("Lane9")
    ...
    .setSize(buttonSize, 55);
  for (int i = 0; i < 7; i++) {
    cp5.addButton("Lane1" + PApplet.parseChar(48+i))
      .setFont(bFont)
...
public void Submit() {
  if (textVal != "Select Lane") {
    ...
    for(int i = 0; i < LANE_TIMES; i++) {
      laneArr[i+1] = PApplet.parseByte(checkbox.getState(i));
      println(PApplet.parseByte(checkbox.getState(i)));
    }
    checkArr(laneArr);
    port.write(laneArr);
    println("Done");
...
public void activateBoxes(byte arr[]) {
...
public void Lane16() {
  ...
  public void settings() {  size(1050, 500);  smooth(); }
